 ...
 * </pre>
 *
 * @author Abhinay Putta and Ben Nguyen
 *
 */
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        //declare lesser to initially be 0
        int lesser = 0;
        /*
         * There's only a need to sift down if a child exists. We only need to
         * check if the left child exists because if the right child exists then
         * the left must also exist with a structure of a heap.
         */
        if (top * 2 + 1 <= last) {
            /*
             * Lesser will hold the index holding the lesser value of the two
             * children. If top*2+2 is greater than the last index then only the
             * left child exists so lesser will be set to the left child.
             *
             * Else if both children exist, lesser will hold the index of the
             * child with the smaller value.
             */
            if (top * 2 + 2 > last) {
                lesser = top * 2 + 1;
            } else if (order.compare(array[top * 2 + 1],
                    (array[top * 2 + 2])) > 0) {
                lesser = top * 2 + 2;
            } else {
                lesser = top * 2 + 1;
            ...
            /*
             * If the value at the index of top is greater than the value of
             * lesser, the entries are switched. Then the value at index of
             * lesser is sifted down
             */
            if (order.compare(array[top], array[lesser]) > 0) {
                exchangeEntries(array, top, lesser);
                siftDown(array, lesser, last, order);
            }
        }
